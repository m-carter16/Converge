...
const NewEventModal: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (selectedUsersDetails?.length !== 0) {
      if (selectedUsersDetails) {
        const newAttendees = selectedUsersDetails
          .map((user) => ({ displayName: user.displayName }))
          .filter((user) => user !== undefined && user !== null) as MicrosoftGraph.User[];
  ...
  const onAttendeesChange = (
    ...
    if (data?.value) {
      const newAttendees = data.value as IDropdownProps[] || [];
      setAttendees(newAttendees
        .map((a) => fullUserData.find((u) => u.displayName === a.header) as MicrosoftGraph.User)
        .filter((a) => !!a).concat(attendees));
    } else {
      setAttendees([]);
    }
  };
        ...
        .then((response) => {
          setAttendeeItems(
            response.users
              ...
              .map((u) => ({ header: u.displayName, content: u.mail })),
          );
