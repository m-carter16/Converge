...
const BookWorkspace: React.FC = () => {
  ...
  const refreshRecommendation = () => {
    ...
    meService.getMyLocation(day.year(), day.month() + 1, day.date())
      .then((recommendation) => {
        setMyRecommendation(recommendation.name);
        if (recommendation.name !== "Remote" && recommendation.name !== "Out of Office" && recommendation.name !== "Unknown") {
          buildingService.getBuildingByDisplayName(recommendation.name).then((building) => {
            if (building) {
  ...
  const refreshRecommended = async () => {
    ...
    meService.getMyLocation(day.year(), day.month() + 1, day.date())
      .then((recommendation) => {
        setMyRecommendation(recommendation.name);
        if (recommendation.name !== "Remote" && recommendation.name !== "Out of Office" && recommendation.name !== "Unknown") {
          const building = buildingsList.find((b) => b.displayName === recommendation.name);
          if (building) {
            getSchedule(building);
            setSelectedBuilding(building);
          }
