...
import WorkingHoursCell from "./WorkingHoursCell";
...
const SelectableTable: React.FC<Props> = (props) => {
  ...
  const { teammates, getErrorMessage, usedAsOpprtunityTable } = props;
  const {
    convergeSettings,
    setConvergeSettings,
  ...
  const handleMenuItemClick = (upn: string | undefined | null) => {
    if (upn) {
      ...
      setConvergeSettings(newSettings).then(() => {
        ...
        if (state.list === TeammateList.MyList && !usedAsOpprtunityTable) {
          getErrorMessage && getErrorMessage(true, 1);
        } else {
          getErrorMessage && getErrorMessage(true, 2);
        }
      });
    }
  ...
  const headers = usedAsOpprtunityTable ? [
    { title: "Name", key: "name", name: "name" },
    { title: "Working hours", key: "workingHours", name: "workingHours" },
    { title: "Time zone", key: "timeZone", name: "timeZone" },
                  ...
                  <Box className={classes.teammates}>
                    {teammates.length ? teammates.map((teammate) => (
                      ...
                      >
                        ...
                        />
                        <Table.Cell
                          // hidden={usedAsOpprtunityTable}
                          key="1-2"
                          accessibility={gridCellBehavior}
                          className={classes.tableCell}
                          content={(
                            <WorkingHoursCell
                              userId={teammate?.user?.userPrincipalName as string}
                            />
                          )}
                        />
                        <Table.Cell
                        ...
                        <Table.Cell
                          ...
                          content={isError ? (
                            ...
                            <AvailableTimesCell
                              ...
                              usedAsOpprtunityTable={usedAsOpprtunityTable}
                            />
                          )}
                        />
                        <Table.Cell
