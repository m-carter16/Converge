...
server.post(
  ...
  async (req, res) => {
    ...
    const member = await bot.notification.findMember(
      async (m) => m.account.email === req.body.recipient
    );

    if (req.body.type === "real-time") {
      await member?.sendAdaptiveCard(
        AdaptiveCards.declare<CardData>(notificationTemplate).render({
          title: "Opportunity to connect!",
          appName: "Converge App Notification",
          description: `${req.body.opportunityName} will be in the ${req.body.location} office on ${req.body.date}. Please use button below to schedule time using Converge.`,
          notificationUrl: req.body.convergeLink,
        })
      );
    }
    ...
    if (req.body.type !== "real-time") {
      await member?.sendAdaptiveCard(
        AdaptiveCards.declare<CardData>(notificationTemplate).render({
          title: "Opportunity to connect!",
          appName: "Converge App Notification",
          description: `There are several opportunities for you to connect in-person. Please use button below to schedule time using Converge.`,
          notificationUrl: "req.body.convergeLink",
        })
      );
    }
